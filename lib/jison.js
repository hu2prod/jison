...
generator.constructor = function Jison_Generator (grammar, opt) {
    ...
    this.suppressConflicts = options.suppressConflicts || false;
...
lookaheadMixin.followSets = function followSets () {
    ...
    while(cont) {
        ...
        for(var j=0,len=list.length;j<len;j++) {
            ...
            var list2 = production.handle;
            for (var i=0,len2=list2.length;i<len2;++i) {
              var t = production.handle[i];
                ...
                if (i === production.handle.length+1 && bool) {
                    ...
                    if (self.nullable(part) && bool) {
                        ...
                        set.append(nonterminals[production.symbol].follows);
                    }
                }
                oldcount = nonterminals[t].follows.length;
                
                var dst_list = nonterminals[t].follows;
                var dst_hash = dst_list._hash || {};
                if (!dst_list._hash) {
                  dst_list._hash = dst_hash;
                  for(var ii=0,len3=dst_list.length;ii<len3;ii++) {
                    var item = dst_list[ii];
                    dst_hash[item] = true;
                  }
                }
                for(var ii=0,len3=set.length;ii<len3;ii++) {
                  var item = set[ii];
                  if (dst_hash[item]) continue;
                  dst_list.push(item);
                  dst_hash[item] = true;
                }
    ...
    // RHS
        ...
        for (var i=0,t;t=symbol[i];++i) {
            if (!this.nonterminals[t]) {
                ...
                firsts.uappend(this.nonterminals[t].first);
            }
            if (!this.nullable(t))
                break;
        }
...
lookaheadMixin.firstSets = function firstSets () {
    ...
    while(cont) {
        ...
        for (symbol in nonterminals) {
            firsts = [];
            var list = nonterminals[symbol].productions._items;
            for(var i=0,len=list.length;i<len;i++) {
              var production = list[i];
              firsts.uappend(production.first);
            }
...
lrGeneratorMixin.canonicalCollection = function canonicalCollection () {
    ...
    while (marked !== states.size()) {
        ...
        markedSymbols = {};
        itemSet.forEach(function CC_itemSet_forEach (item,idx) {
...
lrGeneratorMixin.parseTable = function parseTable (itemSets) {
        ...
        itemSet.reductions.forEach(function (item, j) {
            ...
            terminals.forEach(function (stackSymbol) {
                ...
                if (action || action && action.length) {
                    ...
                    if (sol.bydefault) {
                        self.conflicts++;
                        if (!self.DEBUG && !self.suppressConflicts) {
    ...
    if (!self.DEBUG && self.conflicts > 0 && !self.suppressConflicts) {
...
var lrGeneratorDebug = {
    ...
    afterparseTable: function () {
        var self = this;
        if (this.conflicts > 0 && !this.suppressConflicts) {
    ...
    unionLookaheads: function LALR_unionLookaheads () {
        ...
        states.forEach(function union_states_forEach (i) {
            ...
            state.reductions.forEach(function union_reduction_forEach (item) {
                ...
                var list2 = state.goes[item.production.handle.join(' ')];
                for(var i2=0,len2=list2.length;i2<len2;i2++) {
                    var symbol = list2[i2];
                    var list3 = newg.nonterminals[symbol].follows;
                    for(var i3=0,len3=list3.length;i3<len3;i3++) {
                      var symbol3 = list3[i3];
                      var terminal = terms_[symbol3];
                      if (!follows[terminal]) {
